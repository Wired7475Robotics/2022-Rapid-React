 ...
 * (e.g., 6WD or 8WD). This class takes a Motor per side. For four and six motor
 * drivetrains, construct and pass in {@link
 * edu.wpi.first.wpilibj.motorcontrol.MotorGroup} instances as follows.
 *
 * <p>Four motor drivetrain:
 *
 * <pre><code>
 * public class Robot {
 *   Motor m_frontLeft = new PWMVictorSPX(1);
 *   Motor m_rearLeft = new PWMVictorSPX(2);
 *   MotorGroup m_left = new MotorGroup(m_frontLeft, m_rearLeft);
 *
 *   Motor m_frontRight = new PWMVictorSPX(3);
 *   Motor m_rearRight = new PWMVictorSPX(4);
 *   MotorGroup m_right = new MotorGroup(m_frontRight, m_rearRight);
 *
 ...
 * public class Robot {
 *   Motor m_frontLeft = new PWMVictorSPX(1);
 *   Motor m_midLeft = new PWMVictorSPX(2);
 *   Motor m_rearLeft = new PWMVictorSPX(3);
 *   MotorGroup m_left = new MotorGroup(m_frontLeft, m_midLeft, m_rearLeft);
 *
 *   Motor m_frontRight = new PWMVictorSPX(4);
 *   Motor m_midRight = new PWMVictorSPX(5);
 *   Motor m_rearRight = new PWMVictorSPX(6);
 *   MotorGroup m_right = new MotorGroup(m_frontRight, m_midRight, m_rearRight);
 *
 *   DifferentialDrive m_drive = new DifferentialDrive(m_left, m_right);
 * }
 * </code></pre>
...
public class DifferentialDrive extends RobotDriveBase implements Sendable, AutoCloseable {
   ...
   * edu.wpi.first.wpilibj.motorcontrol.MotorGroup}. If a motor needs to be inverted, do
  ...
  public void arcadeDrive(double xSpeed, double zRotation, boolean squareInputs) {
    ...
    m_leftMotor.run(speeds.left * m_maxOutput);
    m_rightMotor.run(speeds.right * m_maxOutput);
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("Left Motor Speed", m_leftMotor::get, m_leftMotor::run);
    builder.addDoubleProperty(
        "Right Motor Speed", () -> m_rightMotor.get(), x -> m_rightMotor.run(x));
  }
}